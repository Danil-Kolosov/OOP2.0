...
namespace Lab14
{
    class Programm
    {
        public static void Main() 
        {
            ...
            //var qiExtesio = collectionDictionaryZoo.Values.Select(section => section).ToArray().SelectMany(animal => animal).Where(animal => ((Animal)animal).Weight>50)
            ...
            Console.WriteLine("\n\n\nСоздадим объект зоопарк MyNewCollection и с помощью метода расширения этой коллекции SelectionOnCondition выведем список всех рептилий в зоопарке");
            MyNewCollection<string, Animal> zoo = new MyNewCollection<string, Animal>(3);
            zoo.Name = "Зоопарк";
            zoo.Add("Рептилии", new Animal(13, 5, 6, "Анаконда"));
            zoo.Add("Рептилии", new Animal(50, 3, 34, "Крокодил"));
            ...
            Console.WriteLine("Теперь найдем среднее и максимальное значения возраста всех животных");

            double averageAge = zoo.MyAverage(item => item.Value.Age);
            double maxAge = zoo.MyAggregate(item => item.Value.Age, (a,b) => (a > b) ? a : b);
            Console.WriteLine($"Срeдний возраст равен {averageAge}, при этом максимальный - {maxAge}");

            Console.WriteLine("Отсортируем по уменьшению роста");
            //var zooOrderByHeight = zoo.Select(pair => pair).Where(pair => pair.Value is Animal).Order
            var zooOrderByHeight = zoo.Order((item) => { if (item.Value == null) return 0; return item.Value?.Height ?? 0; }, true);
            //var zooOrderByHeight = zoo.Order(item => item.Value.Height/*(item.Value != null ) ? item.Value.Height : 0*/);
            //zooOrderByHeight.Print();
